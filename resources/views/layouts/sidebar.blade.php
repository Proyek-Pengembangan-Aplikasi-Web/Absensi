...
<nav class="bg-dark text-white p-3" style="width: 250px; height: 100vh;">
    <ul class="nav flex-column mt-4">
        ...
        @else
            @php
                $pelajaran = App\Models\Jadwal::where('id_user', Auth::user()->id)->get();
            @endphp

            <li class="nav-item">
                <a class="nav-link text-white active" href="#index" data-bs-toggle="collapse" role="button" aria-expanded="false" aria-controls="index">
                    Jadwal Saya
                </a>
                <ul class="nav flex-column collapse submenu" id="index">
                    @foreach ($pelajaran as $pljrn)
                        <li class="nav-item">
                            <a class="nav-link text-white" href="">{{ $pljrn->pelajaran->name }}</a>
                        </li>
                    @endforeach
                </ul>
            </li>
